...
local function Rcheck(Rcount)
        if tonumber(Rcount) then
                return true
        elseif Rcount == "ALL" then
                return true
...
local function convert(Rcount)
        if Rcount == "ALL" or Rcount == "all" or Rcount == "All" then
                return "ALL"
        elseif Rcount == "PURGE" or Rcount == "purge" or Rcount == "Purge" then
                return "PURGE"
        elseif Rcount == "CHECK" or Rcount == "check" or Rcount == "Check" then
                return 0
        else
...
local function take (dir,idef,frominv,frompos,frominvname,stack,index,Rcount)--return bool
        local item = stack:take_item(Rcount)
        frominv:set_stack(frominvname, index, stack)
        if idef.on_metadata_inventory_take then
            idef.on_metadata_inventory_take(frompos, "main", index, item, fakePlayer)
        else
            return false
        end
...
local function extract(pos,dir,Sitem,Rcount)
                ...
                for index, stack in ipairs(frominv:get_list(frominvname)) do
                        ...
                        if not stack:is_empty() then
                                ...
                                if not tonumber(Rcount) then
                                        if Rcount == "ALL" then
                                                if take(dir,idef,frominv,frompos,frominvname,stack,index,count) then
                                        ...
                                        elseif Rcount < count then
                                                if take(dir,idef,frominv,frompos,frominvname,stack,index,Rcount) then
        ...
        elseif tonumber(Sitem) then--grab a specific slot
                ...
                if not stack:is_empty() then
                        if not tonumber(Rcount) then
                                if Rcount == "ALL" then
                                        if take(dir,idef,frominv,frompos,frominvname,stack,Sitem,count) then
                                                local R = formatted(count)..stack:get_name()
                                                return R
                                        else
                                                return "ERROR"
                                ...
                                elseif Rcount > count then
                                        ...
                                        return R
                                elseif Rcount <= count then
                                        if take(dir,idef,frominv,frompos,frominvname,stack,Sitem,Rcount) then
                                                local R = formatted(count)..stack:get_name()
                                                return R
                                        else
                                                return "ERROR"
                ...
                for index, stack in ipairs(frominv:get_list(frominvname)) do
                        ...
                        if not stack:is_empty() then
                                if not tonumber(Rcount) then
                                        if Rcount == "PURGE" and Sname == Sitem then
                                                if take(dir,idef,frominv,frompos,frominvname,stack,index,count) then
                                                        purge = purge + count
                                                else
                                                        return "ERROR"
                                        ...
                                        if Rcount == 0 and Sname == Sitem then
                                                Ccheck = Ccheck + count
                                                ...
                                                if Rcount <= every1 then
                                                        ...
                                                        for i, index in ipairs(Icue) do
                                                                ...
                                                                if i < #Icue then
                                                                        ...
                                                                        if take(dir,idef,frominv,frompos,frominvname,stack,index,Rcount - every1) then
                                        ...
                                        elseif Rcount <= count and Sname == Sitem then
                                                if every1 ~= 0 then--we were lacking in the other stacks but the current one has enough..grab the lacking ones first
                                                        ...
                                                        for i,index2 in ipairs(Icue) do
                                                                ...
                                                                if take(dir,idef,frominv,frompos,frominvname,Tstack,index2,Tcount) then
    ...
    effector = {action = function(pos,node,channel,msg)
                ...
                if localchan == channel or channel == "ALL FILTERS" or not localchan == nil or not localchan == "" then
                        ...
                        if Sitem == "SILENT" then
                                ...
                                if silent == 0 then
                                        local digi_return = extract(pos,dir,Sitem,Rcount)
